...
function SalesChart({ bookings, numDays }) {
  ...
  const colors = isDarkMode
    ? {
        totalSales: { stroke: "#466be5", fill: "#4678e5" },
        extrasSales: { stroke: "#22c55e", fill: "#22c55e" },
        text: "#e5e7eb",
        background: "#18212f",
      }
    : {
        totalSales: { stroke: "#467ee5", fill: "#c7d4fe" },
  ...
  return (
    <StyledSalesChart>
      <Heading as="h2">
        Продажи от {format(allDates.at(0), "dd MMMM yyyy", { locale: ru })} до{" "}
        {format(allDates.at(-1), "dd MMMM yyyy", { locale: ru })}
      </Heading>
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={data} height={300} width={700}>
          <XAxis
            dataKey="label"
            tick={{ fill: colors.text, fontSize: "1.2rem" }}
